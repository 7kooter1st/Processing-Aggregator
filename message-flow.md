...
> Важно: разделы с покусочным `Compare` ниже сохранены как описание раннего
> прототипа. В текущей версии Kafka-чанки используются только для OCR:
> распознанные страницы сохраняются в PostgreSQL, после готовности всех страниц
> выполняются единый page-aware diff, детерминированная фильтрация, локальная
> LLM-классификация и публикация одного результата `1/1`. Актуальная схема БД и
> deployment приведены в `../DEPLOYMENT_GUIDE.md`.
